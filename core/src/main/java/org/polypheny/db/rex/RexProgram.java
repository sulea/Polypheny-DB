...
public class RexProgram implements Serializable {
     ...
     * @param conditionExpr Condition on which to filter rows, or null if rows are not to be filtered
    ...
    public AlgWriter collectExplainTerms( String prefix, AlgWriter pw, ExplainLevel level ) {
        ...
        // If a lot of the fields are simply projections of the underlying expression, try to be a bit less verbose.
        int trivialCount = 0;
        ...
        // Print the non-trivial fields with their names as they appear in the output row type.
    ...
    public boolean isValid( Litmus litmus, AlgNode.Context context ) {
        ...
        // If the input row type is a struct (contains fields) then the leading expressions must be references to those fields. But we don't require this if the input row type is, say, a java class.
     ...
     *
     * Like {@link RexUtil#isNull(RexNode)}, null literals are null, and casts of null literals are null. But this method also regards references to null expressions as null.
     *
     ...
     * For example, <code>program.expandLocalRef(program.getCondition())</code> will return the expansion of a program's condition.
     *
     ...
     *
     * @return array of reference counts; the ith element in the returned array is the number of references to getExprList()[i]
     */
    ...
    public RexProgram normalize( RexBuilder rexBuilder, RexSimplify simplify ) {
        // Normalize program by creating program builder from the program, then converting to a program. getProgram does not need to normalize
    ...
    static class Checker extends RexChecker {
        ...
        /**
         * Overrides {@link RexChecker} method, because {@link RexLocalRef} is illegal in most rex expressions, but legal in a program.
         */
        @Override
    ...
    /**
     * Given an expression in a program, creates a clone of the expression with sub-expressions (represented by {@link RexLocalRef}s) fully expanded.
     */
